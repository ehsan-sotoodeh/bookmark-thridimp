...
# load data
#========================================
ctData.df <- read.table('data_visits_out.txt', sep="\t", header=TRUE)
names(ctData.df)
...
ctSummary.df <- ddply(subset(ctData.df,select=c("phase","blk", "ct", "cat2")), .(cat2), summarize, mean = round(mean(ct),2), sd = round(sd(ct),2), se = round(sd(ct)/sqrt(length(ct)),2))
ctSummary.df
...
ct_anova <- ezANOVA(data=ctData.df, dv=ct, wid=ID, within=.(cat2), detailed=TRUE)
ct_anova

ct_anovaErr <- ezANOVA(data=ctData.df, dv=err, wid=ID, within=.(cat2), type = 1, detailed=TRUE)
ct_anovaErr

pairwise.t.test(ctData.df$ct, ctData.df$phase, p.adj = "bonf")
